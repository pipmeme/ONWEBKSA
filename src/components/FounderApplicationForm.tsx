...
const SECTION_META: Record<Section, { labelKey: string; icon: typeof Rocket; color: string }> = {
  welcome: { labelKey: "founderApp.welcome", icon: Sparkles, color: "text-primary" },
  startup: { labelKey: "founderApp.yourStartup", icon: Building2, color: "text-primary" },
  you: { labelKey: "founderApp.aboutYou", icon: User, color: "text-primary" },
  funding: { labelKey: "founderApp.fundingSupport", icon: Banknote, color: "text-primary" },
  final: { labelKey: "founderApp.finalThoughts", icon: MessageSquare, color: "text-primary" },
};
...
const FounderApplicationForm = ({
  ...
  const localizedSectionMeta = useMemo(
    () => ({
      welcome: { ...SECTION_META.welcome, label: t(SECTION_META.welcome.labelKey) },
      startup: { ...SECTION_META.startup, label: t(SECTION_META.startup.labelKey) },
      you: { ...SECTION_META.you, label: t(SECTION_META.you.labelKey) },
      funding: { ...SECTION_META.funding, label: t(SECTION_META.funding.labelKey) },
      final: { ...SECTION_META.final, label: t(SECTION_META.final.labelKey) },
    }),
    [t]
  );

  const localizedSteps = useMemo<Step[]>(() => {
    const stageOptionLabel: Record<string, string> = {
      idea: t("founderApp.stageIdea"),
      mvp: t("founderApp.stageMvp"),
      pre_seed: t("founderApp.stagePreSeed"),
      seed: t("founderApp.stageSeed"),
      series_a: t("founderApp.stageSeriesA"),
      series_b_plus: t("founderApp.stageSeriesBPlus"),
      profitable: t("founderApp.stageProfitable"),
    };

    const roleOptionLabel: Record<string, string> = {
      CEO: t("founderApp.roleCeo"),
      CTO: t("founderApp.roleCto"),
      COO: t("founderApp.roleCoo"),
      CFO: t("founderApp.roleCfo"),
      CPO: t("founderApp.roleCpo"),
      CMO: t("founderApp.roleCmo"),
      Other: t("founderApp.roleOther"),
    };

    const fundingOptionLabel: Record<string, string> = {
      bootstrapped: t("founderApp.bootstrapped"),
      angel: t("founderApp.angel"),
      vc: t("founderApp.vc"),
      grant: t("founderApp.grant"),
      mixed: t("founderApp.mixed"),
    };

    const industryOptionLabel: Record<string, string> = {
      Fintech: t("founderApp.industryFintech"),
      "E-Commerce": t("founderApp.industryEcommerce"),
      SaaS: t("founderApp.industrySaas"),
      AI: t("founderApp.industryAi"),
      Healthcare: t("founderApp.industryHealthcare"),
      EdTech: t("founderApp.industryEdtech"),
      Gaming: t("founderApp.industryGaming"),
      Biotech: t("founderApp.industryBiotech"),
      Consultancy: t("founderApp.industryConsultancy"),
      "Travel & Tourism": t("founderApp.industryTravel"),
      Logistics: t("founderApp.industryLogistics"),
      "Real Estate": t("founderApp.industryRealEstate"),
      "Food & Beverage": t("founderApp.industryFood"),
      Other: t("founderApp.other"),
    };
    ...
    return STEPS.map((step) => {
      switch (step.id) {
        case "welcome":
          return {
            ...step,
            question: t("founderApp.getFeaturedTitle"),
            subtitle: t("founderApp.getFeaturedSubtitle"),
          };
        case "feature_type":
          return {
            ...step,
            question: t("founderApp.howFeatured"),
            subtitle: t("founderApp.howFeaturedSubtitle"),
            options: [
              { value: "rising_founder", label: t("founderApp.risingOption") },
              { value: "founder_story", label: t("founderApp.storyOption") },
            ],
          };
        case "company_name":
          return {
            ...step,
            question: t("founderApp.startupName"),
            placeholder: t("founderApp.startupNamePlaceholder"),
          };
        case "one_liner":
          return {
            ...step,
            question: t("founderApp.describeOneLine"),
            subtitle: t("founderApp.elevatorPitch"),
            placeholder: t("founderApp.helpWhoPlaceholder"),
          };
        case "industry":
          return {
            ...step,
            question: t("founderApp.whatIndustry"),
            options: (step.options || []).map((option) => ({
              ...option,
              label: industryOptionLabel[option.value] || option.label,
            })),
          };
        case "stage":
          return {
            ...step,
            question: t("founderApp.whatStage"),
            options: (step.options || []).map((option) => ({
              ...option,
              label: stageOptionLabel[option.value] || option.label,
            })),
          };
        case "startup_details":
          return {
            ...step,
            question: t("founderApp.moreDetails"),
            subtitle: t("founderApp.moreDetailsSubtitle"),
            fields: [
              { field: "founded_date", label: t("founderApp.whenStart"), placeholder: t("founderApp.whenStartPlaceholder") },
              { field: "team_size", label: t("founderApp.teamSize"), placeholder: t("founderApp.teamSizePlaceholder") },
              { field: "company_website", label: t("founderApp.websiteLabel"), placeholder: t("founderApp.websitePlaceholder") },
              { field: "social_media", label: t("founderApp.socialMedia"), placeholder: t("founderApp.socialMediaPlaceholder") },
            ],
          };
        case "num_cofounders":
          return {
            ...step,
            question: t("founderApp.howManyFounders"),
            subtitle: t("founderApp.includingYourself"),
            options: ["1", "2", "3", "4", "5"].map((count) => ({
              value: count,
              label: count === "1" ? t("founderApp.soloFounder") : `${count} ${t("founderApp.founders")}`,
            })),
          };
        case "founder_role":
          return {
            ...step,
            question: t("founderApp.yourRole"),
            options: (step.options || []).map((option) => ({
              ...option,
              label: roleOptionLabel[option.value] || option.label,
            })),
          };
        case "cofounders":
          return {
            ...step,
            question: t("founderApp.tellCofounders"),
            subtitle: t("founderApp.cofounderSubtitle"),
          };
        case "founder_name":
          return {
            ...step,
            question: t("founderApp.yourFullName"),
            subtitle: t("founderApp.leadFounder"),
            placeholder: t("founderApp.founderNamePlaceholder"),
          };
        case "email":
          return {
            ...step,
            question: t("founderApp.yourEmail"),
            subtitle: t("founderApp.emailOnlyFeature"),
            placeholder: t("founderApp.emailPlaceholder"),
          };
        case "personal_details":
          return {
            ...step,
            question: t("founderApp.moreAboutYou"),
            subtitle: t("founderApp.moreAboutYouSubtitle"),
            fields: [
              { field: "phone", label: t("founderApp.phone"), placeholder: t("founderApp.phonePlaceholder") },
              { field: "age", label: t("founderApp.ageLabel"), placeholder: t("founderApp.agePlaceholder") },
              { field: "education", label: t("founderApp.educationLabel"), placeholder: t("founderApp.educationPlaceholder") },
              { field: "linkedin_url", label: t("founderApp.linkedin"), placeholder: t("founderApp.linkedinPlaceholder") },
            ],
          };
        case "location":
          return {
            ...step,
            question: t("founderApp.whereBased"),
            fields: [
              { field: "country", label: t("founderApp.country"), placeholder: t("founderApp.countryPlaceholder"), required: true },
              { field: "city", label: t("founderApp.city"), placeholder: t("founderApp.cityPlaceholder") },
            ],
          };
        case "gender":
          return {
            ...step,
            question: t("founderApp.yourGender"),
            options: [
              { value: "Male", label: t("founderApp.male") },
              { value: "Female", label: t("founderApp.female") },
            ],
          };
        case "is_incubated":
          return {
            ...step,
            question: t("founderApp.partOfIncubator"),
            options: [
              { value: "yes", label: t("founderApp.yes") },
              { value: "no", label: t("founderApp.notYet") },
            ],
          };
        case "incubator_name":
          return {
            ...step,
            question: t("founderApp.whichOne"),
            subtitle: t("founderApp.nameIncubator"),
            placeholder: t("founderApp.incubatorPlaceholder"),
          };
        case "funding_type":
          return {
            ...step,
            question: t("founderApp.howFunded"),
            options: (step.options || []).map((option) => ({
              ...option,
              label: fundingOptionLabel[option.value] || option.label,
            })),
          };
        case "funding_details":
          return {
            ...step,
            question: t("founderApp.fundingDetails"),
            subtitle: t("founderApp.fundingDetailsSubtitle"),
            fields: [
              { field: "funding_amount", label: t("founderApp.amountRaised"), placeholder: t("founderApp.amountRaisedPlaceholder") },
              { field: "grants_received", label: t("founderApp.grantsReceived"), placeholder: t("founderApp.grantsReceivedPlaceholder") },
              { field: "revenue", label: t("founderApp.monthlyRevenue"), placeholder: t("founderApp.monthlyRevenuePlaceholder") },
            ],
          };
        case "message":
          return {
            ...step,
            question: t("founderApp.anythingElse"),
            subtitle: t("founderApp.anythingElseSubtitle"),
            placeholder: t("founderApp.sharePlaceholder"),
          };
        default:
          return step;
      }
  ...
  const visibleSteps = localizedSteps.filter(
    (step) => !step.conditional || step.conditional(form)
  );
  const step = visibleSteps[currentStep];
  const totalSteps = visibleSteps.length;
  const progress = ((currentStep + 1) / totalSteps) * 100;
  const sectionMeta = step ? localizedSectionMeta[step.section] : null;
  ...
  const next = useCallback(() => {
    if (!canProceed()) {
      toast.error(t("founderApp.fillRequired"));
      return;
    }
  ...
  }, [canProceed, currentStep, t, totalSteps]);
  ...
  const handleSubmit = useCallback(async () => {
    if (!canProceed()) {
      toast.error(t("founderApp.fillRequiredShort"));
      return;
    }
    setSubmitting(true);
    ...
    setSubmitting(false);
    if (error) {
      toast.error(t("founderApp.somethingWrong"));
    } else {
      setSubmitted(true);
    }
  }, [canProceed, cofounders, defaultFeatureType, form, t]);
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-background flex items-center justify-center">
        ...
        >
          ...
          <h2 className="font-display text-3xl sm:text-4xl font-bold mb-4">{t("founderApp.youreIn")}</h2>
          <p className="text-muted-foreground text-base sm:text-lg leading-relaxed mb-4">
            {t("founderApp.receivedApp")} <span className="font-semibold text-foreground">{t("founderApp.hours48")}</span> {t("founderApp.ifSelected")}
          </p>
          <p className="text-muted-foreground text-sm mb-8">
            {t("founderApp.keepBuilding")}
          </p>
          <Button onClick={handleClose} size="lg" className="gradient-bg text-primary-foreground px-8">
            {t("founderApp.backToStories")}
          </Button>
        </motion.div>
      </div>
    );
      ...
      return (
        <div className="space-y-6">
          <div className="flex gap-2 flex-wrap">
            {sections.map((s) => {
              const meta = localizedSectionMeta[s];
          ...
          <p className="text-xs text-muted-foreground">
            {t("founderApp.proTip")} <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">Enter</kbd> {t("founderApp.enterAdvance")} · <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">Backspace</kbd> {t("founderApp.backspaceBack")} · {t("founderApp.letterKeys")} <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">A</kbd> <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">B</kbd> <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">C</kbd> {t("founderApp.pickOptions")}
          </p>
        </div>
      );
    }
          ...
          {hasOtherOption && isOtherSelected && (
            <div className="mt-4">
              <Input
                ...
                placeholder={t("founderApp.typeRolePlaceholder")}
              ...
              <div className="mt-3 flex items-center gap-2.5">
                ...
                >
                  {t("founderApp.ok")} <Check className="h-4 w-4" />
                </Button>
              </div>
            </div>
          )}
        ...
        <div className="space-y-4 w-full max-w-lg">
          {Array.from({ length: count }).map((_, i) => (
            <div key={i} className="rounded-xl border border-border p-4 space-y-3 bg-card">
              <p className="text-sm font-semibold text-foreground">{t("founderApp.cofounder")} {i + 1}</p>
              <Input
                placeholder={t("founderApp.fullNameLabel")}
              ...
              >
                <option value="">{t("founderApp.selectRole")}</option>
                {["CEO", "CTO", "COO", "CFO", "CPO", "CMO"].map((r) => (
                  <option key={r} value={r}>{r}</option>
                ))}
                <option value="custom">{t("founderApp.otherTypeRole")}</option>
              </select>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <Input
                  placeholder={t("founderApp.ageLabel")}
                  value={cofounders[i]?.age || ""}
                  onChange={(e) => updateCofounder(i, "age", e.target.value)}
                  className="h-11 text-sm"
                />
                <Input
                  placeholder={t("founderApp.educationLabel")}
    ...
    <div className="fixed inset-0 z-[100] bg-background flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 sm:px-8 py-3.5 border-b border-border">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-display font-bold text-sm">{t("founderApp.getFeatured")}</span>
        </div>
          ...
          >
            ...
            {showActionButtons && (
              <div className="mt-8 flex items-center gap-2.5 flex-wrap">
                ...
                >
                  {submitting ? t("founderApp.submitting") : isLastStep ? (
                    <>{t("founderApp.submit")} <Check className="h-4 w-4" /></>
                  ) : (
                    <>{t("founderApp.ok")} <Check className="h-4 w-4" /></>
                  )}
                </Button>
                {isOptionalStep && !isLastStep && (
                  ...
                  >
                    {t("founderApp.skip")} <SkipForward className="h-3.5 w-3.5" />
                  </Button>
                )}
                <span className="text-xs text-muted-foreground hidden sm:flex items-center gap-1 ml-1">
                  {t("founderApp.pressEnter")} <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">Enter ↵</kbd>
                </span>
              </div>
            )}
            {showWelcomeCTA && (
              <div className="mt-8">
                ...
                >
                  {t("founderApp.letsGo")} <ArrowRight className="h-5 w-5" />
                </Button>
                <p className="text-xs text-muted-foreground mt-3">
                  {t("founderApp.orPress")} <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono text-[0.6rem]">Enter ↵</kbd>
                </p>
              </div>
            )}
          </motion.div>
      ...
      <div className="flex items-center justify-between px-5 sm:px-8 py-3.5 border-t border-border">
        ...
        >
          ...
          <span className="hidden sm:inline">{t("founderApp.back")}</span>
        </Button>
        ...
        <Button
          ...
          className="gap-1.5 text-muted-foreground h-9"
        >
          <span className="hidden sm:inline">{isLastStep ? t("founderApp.submit") : t("founderApp.next")}</span>
