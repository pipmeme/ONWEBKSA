...
const FreeKickstart = () => {
  ...
  const helpOptions = [
    { value: "Website", label: t("freeKickstart.website") },
    { value: "Instagram & Social Media", label: t("freeKickstart.instagram") },
    { value: "Logo & Branding", label: t("freeKickstart.logoBranding") },
    { value: "Posters & Flyers", label: t("freeKickstart.postersFlyers") },
    { value: "Business Cards", label: t("freeKickstart.businessCards") },
    { value: "Other", label: t("freeKickstart.other") },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.help_needed.length === 0) {
      toast({ title: t("freeKickstart.selectHelp"), variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast({ title: t("freeKickstart.somethingWrong"), description: t("freeKickstart.tryAgain"), variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <section className="min-h-[60vh] flex items-center justify-center py-16">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-md mx-auto px-5">
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-bold mb-3">{t("freeKickstart.youreIn")} 🎉</h1>
          <p className="text-muted-foreground text-base mb-1">
            {t("freeKickstart.wellReview")}
          </p>
          <p className="text-muted-foreground text-sm">{t("freeKickstart.keepBuilding")}</p>
        </motion.div>
      </section>
    );
  }
        ...
        <div className="max-w-lg mx-auto">
          ...
          <motion.div initial="hidden" animate="visible" className="text-center mb-8 sm:mb-10">
            <motion.div variants={fadeUp} custom={0} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent text-accent-foreground text-sm font-medium mb-5">
              <Rocket className="h-4 w-4" /> {t("freeKickstart.freeNoCatch")}
            </motion.div>
            <motion.h1 variants={fadeUp} custom={1} className="font-display text-3xl sm:text-4xl md:text-5xl font-bold mb-4 leading-tight">
              {t("freeKickstart.wellBuild")} <br className="sm:hidden" /><span className="gradient-text">{t("freeKickstart.firstStep")}</span>
            </motion.h1>
            <motion.p variants={fadeUp} custom={2} className="text-muted-foreground text-base sm:text-lg leading-relaxed">
              {t("freeKickstart.desc")} <strong>{t("freeKickstart.completelyFree")}</strong>.
            </motion.p>
          </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={4}>
            <form onSubmit={handleSubmit} className="space-y-4 bg-card border border-border/60 rounded-2xl p-5 sm:p-7 shadow-sm">
              <Input
                placeholder={t("freeKickstart.yourName")}
                value={form.name}
              ...
              />
              <Input
                placeholder={t("freeKickstart.whatsapp")}
                value={form.whatsapp}
              ...
              <Input
                type="email"
                placeholder={t("freeKickstart.emailAddress")}
                value={form.email}
              ...
              />
              <Textarea
                placeholder={t("freeKickstart.tellUsAbout")}
                rows={3}
              ...
              <div>
                <p className="text-sm text-muted-foreground mb-2">{t("freeKickstart.whatHelp")}</p>
                  ...
                  {helpOptions.map((option) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                        form.help_needed.includes(option.value)
              ...
              >
                {submitting ? t("freeKickstart.sendingDots") : t("freeKickstart.sendFree")} {!submitting && <Send className="ml-2 h-4 w-4" />}
              </Button>
            </form>
          </motion.div>
        </div>
