...
/* ─── Stage icons & colors (labels resolved inside component with t()) ─── */

const stageIcons: Record<Stage, { icon: typeof Building2; color: string }> = {
  "pre-launch": { icon: Lightbulb, color: "from-amber-500 to-yellow-400" },
  "misa-license": { icon: BadgeCheck, color: "from-rose-500 to-orange-400" },
  "company-registration": { icon: Building2, color: "from-violet-500 to-indigo-400" },
  "hiring-compliance": { icon: Users, color: "from-emerald-500 to-teal-400" },
  "banking-finance": { icon: Landmark, color: "from-sky-500 to-blue-400" },
  "visas-iqama": { icon: Globe, color: "from-pink-500 to-fuchsia-400" },
  "ongoing-compliance": { icon: Shield, color: "from-cyan-500 to-teal-400" },
};
...
const FoundersPlaybook = () => {
  ...
  const stageConfig = useMemo((): Record<Stage, { label: string; icon: typeof Building2; color: string }> => ({
    "pre-launch": { label: t("playbook.preLaunch"), ...stageIcons["pre-launch"] },
    "misa-license": { label: t("playbook.misaLicense"), ...stageIcons["misa-license"] },
    "company-registration": { label: t("playbook.companyRegistration"), ...stageIcons["company-registration"] },
    "hiring-compliance": { label: t("playbook.hiringCompliance"), ...stageIcons["hiring-compliance"] },
    "banking-finance": { label: t("playbook.bankingFinance"), ...stageIcons["banking-finance"] },
    "visas-iqama": { label: t("playbook.visasIqama"), ...stageIcons["visas-iqama"] },
    "ongoing-compliance": { label: t("playbook.ongoingCompliance"), ...stageIcons["ongoing-compliance"] },
  }), [t]);

  const severityConfig = useMemo((): Record<Severity, { label: string; bg: string; text: string; border: string }> => ({
    critical: { label: t("playbook.critical"), bg: "bg-red-500/10", text: "text-red-700 dark:text-red-400", border: "border-red-500/20" },
    warning: { label: t("playbook.important"), bg: "bg-amber-500/10", text: "text-amber-700 dark:text-amber-400", border: "border-amber-500/20" },
    info: { label: t("playbook.goodToKnow"), bg: "bg-sky-500/10", text: "text-sky-700 dark:text-sky-400", border: "border-sky-500/20" },
  }), [t]);
  ...
  return (
    <>
        ...
        <div className="px-4 sm:container py-6">
          <div className="flex flex-wrap gap-6 sm:gap-10 justify-center text-sm">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">{t("playbook.realInsights")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-foreground">7</p>
              <p className="text-muted-foreground">{t("playbook.journeyStages")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-foreground">
                {playbookEntries.reduce((s, e) => s + e.commonMistakes.length, 0)}+
              </p>
              <p className="text-muted-foreground">{t("playbook.mistakesDocumented")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-foreground">
                {playbookEntries.reduce((s, e) => s + e.proTips.length, 0)}+
              </p>
              <p className="text-muted-foreground">{t("playbook.proTipsCount")}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="px-4 sm:container">
          ...
          {searchQuery && (
            <p className="text-sm text-muted-foreground mt-3">
              {filteredEntries.length} {filteredEntries.length !== 1 ? t("playbook.searchResultsPlural", { count: filteredEntries.length }) : t("playbook.searchResults", { count: filteredEntries.length })} &ldquo;{searchQuery}&rdquo;
              {selectedStage !== "all" && ` ${t("playbook.inStage")} ${stageConfig[selectedStage].label}`}
            </p>
          )}
        </div>
      </section>
          ...
          <AnimatePresence mode="wait">
            {filteredEntries.length === 0 ? (
              ...
              >
                ...
                <p className="text-lg font-medium text-muted-foreground">{t("playbook.noResults")}</p>
                <p className="text-sm text-muted-foreground/70">{t("playbook.noResultsDesc")}</p>
              </motion.div>
            ) : (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-4 mt-4">
                          <div className="flex items-center gap-1.5 text-xs">
                            ...
                            <span className="text-muted-foreground">{t("playbook.officialTimeline")}:</span>
                            <span className="line-through text-muted-foreground/50">{entry.officialTimeline}</span>
                            <span className="text-primary font-medium">→ {t("playbook.realTimeline")}: {entry.realTimeline}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-xs">
                            <DollarSign className="h-3.5 w-3.5 text-muted-foreground" />
                            <span className="text-muted-foreground">{t("playbook.officialCost")}:</span>
                            <span className="line-through text-muted-foreground/50">{entry.officialCost}</span>
                            <span className="text-primary font-medium">→ {t("playbook.realCost")}: {entry.realCost}</span>
                          </div>
                        </div>
                      </button>
                              ...
                              {entry.rejectionReasons && entry.rejectionReasons.length > 0 && (
                                <div>
                                  <h4 className="font-display text-sm font-bold mb-3 flex items-center gap-2">
                                    ...
                                    {t("playbook.topRejectionReasons")}
                                  </h4>
                                  <ul className="space-y-2">
      ...
      <section className="py-12 sm:py-16 border-t border-border/40 bg-muted/30">
        <div className="px-4 sm:container">
          <h3 className="font-display text-lg font-bold mb-6">{t("playbook.deepDiveGuides")}</h3>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                >
                  <a href="mailto:help@foundersksa.com">
                    <Mail className="mr-2 h-4 w-4" /> {t("playbook.shareExperience")}
                  </a>
                </Button>
                <Button
                  asChild
